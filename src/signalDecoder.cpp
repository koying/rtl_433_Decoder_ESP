...
void rtl_433_Decoder::_setDebug(int debug) {
  rtlVerbose = debug;
...
#  endif
...
#endif
#ifdef MEMORY_DEBUG
    logprintfLn(LOG_INFO, "Pre run_%s_demods: %d", thistask->_ookModulation ? "OOK" : "FSK", ESP.getFreeHeap());
#endif
    ...
    if (events == 0) {
#ifdef RTL_ANALYZER
      pulse_analyzer(rtl_pulses, thistask->_ookModulation ? 1 : 2);
#endif
...
//                "rssiThreshold", "", DATA_INT,    rssiThreshold,
    ...
    if (events > 0) {
      ...
      logprintfLn(LOG_DEBUG, "Process rtl_433_DecoderTask stack free: %u",
                  uxTaskGetStackHighWaterMark(thistask->rtl_433_DecoderHandle));
      alogprintfLn(LOG_INFO, " ");
    }
#endif
#ifdef MEMORY_DEBUG
